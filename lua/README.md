...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Chart()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local currency, err = client:Currency():load({ from = "example", to = "example" })
if err then error(err) end
```
...
local result, err = client:Currency():load({ from = "example", to = "example" })
...
```lua
local currency, err = client:Currency():load({ from = "from", to = "to" })
```
...
```
lua/
├── kekkai-currency_sdk.lua    -- Main SDK module
...
The main module (`kekkai-currency_sdk`) exports the SDK constructor
