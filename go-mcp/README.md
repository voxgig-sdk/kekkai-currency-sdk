...
[MCP](https://modelcontextprotocol.io) server exposing the KekkaiCurrency SDK as
two agent tools — `kekkai-currency_list` and `kekkai-currency_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kekkai-currency-mcp)
make build

# 2. Provide credentials via the environment
export KEKKAI_CURRENCY_APIKEY=sk_live_xxx
...
```jsonc
// kekkai-currency_list: first page of records
{ "entity": "chart" }
{ "entity": "chart", "query": { } }

// kekkai-currency_load: one record by id
{ "entity": "currency", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kekkai-currency-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kekkai-currency \
     -- "$PWD"/dist/*/kekkai-currency-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kekkai-currency_list` and `kekkai-currency_load` tools now appear
   in new sessions. Ask the agent to *"list chart using kekkai-currency"*
   and it calls `kekkai-currency_list` with `{"entity":"chart"}`.
...
```sh
export KEKKAI_CURRENCY_APIKEY=sk_live_xxx            # API key
export KEKKAI_CURRENCY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kekkai-currency_list` tool
...
### Call the `kekkai-currency_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kekkai-currency_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kekkai-currency_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KEKKAI_CURRENCY_APIKEY` | API key sent with every request. |
| `KEKKAI_CURRENCY_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kekkai-currency_load","arguments":{"entity":"currency","query":{"id":1}}}}'
```
