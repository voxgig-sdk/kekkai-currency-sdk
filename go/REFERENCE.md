# KekkaiCurrency Golang SDK Reference

Complete API reference for the KekkaiCurrency Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *KekkaiCurrencySDK`
...
#### `Chart(data map[string]any) KekkaiCurrencyEntity`

Create a new `Chart` entity instance. Pass `nil` for no initial data.

#### `Currency(data map[string]any) KekkaiCurrencyEntity`

Create a new `Currency` entity instance. Pass `nil` for no initial data.

#### `Metadata(data map[string]any) KekkaiCurrencyEntity`
...
```go
result, err := client.Currency(nil).Load(map[string]any{"from": "from", "to": "to"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
