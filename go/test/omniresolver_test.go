...
import (
	...
	omni "github.com/voxgig-sdk/kekkai-currency-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KekkaiCurrencyError); is {
				out := map[string]any{
					"name":    "KekkaiCurrencyError",
...
func omniCtx(arg any, client *sdk.KekkaiCurrencySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KekkaiCurrencySDK); is {
			client = live
