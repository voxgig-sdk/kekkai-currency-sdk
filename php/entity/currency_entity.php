...
// KekkaiCurrency SDK Currency entity
...
class CurrencyEntity
{
    ...
    {
        if ($args) {
            $this->_data = KekkaiCurrencyHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = KekkaiCurrencyHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * Load a single Currency.
     *
     * @param CurrencyLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CurrencyLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Currency|array The loaded Currency as an assoc-array at the
     *   SDK boundary; throws KekkaiCurrencyError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KekkaiCurrencyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
