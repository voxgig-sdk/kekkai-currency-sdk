...
// KekkaiCurrency SDK retry feature
...
class KekkaiCurrencyRetryFeature extends KekkaiCurrencyBaseFeature
{
    ...
    public function init(KekkaiCurrencyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KekkaiCurrencyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KekkaiCurrencyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KekkaiCurrencyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
