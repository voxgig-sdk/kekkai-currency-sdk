...
// KekkaiCurrency SDK feature test
//
...
class FtCtrl extends KekkaiCurrencyControl
{
...
class FtClient
{
    ...
    private KekkaiCurrencyUtility $utility;

    public function __construct(KekkaiCurrencyUtility $utility, array $options)
    {
    ...
    public function get_utility(): KekkaiCurrencyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KekkaiCurrencyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KekkaiCurrencyContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KekkaiCurrencyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KekkaiCurrencyConfig::make_feature($name);
    ...
    private static function buildUrl(KekkaiCurrencySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KekkaiCurrencyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KekkaiCurrencyResponse($fetched);
        }
    ...
    private function fail(KekkaiCurrencyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KekkaiCurrencyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
