...
//    only - a typed KekkaiCurrencyContext there would make every ctx
...
final class KekkaiCurrencyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KekkaiCurrencyContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new KekkaiCurrencySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new KekkaiCurrencyResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new KekkaiCurrencyError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new KekkaiCurrencyResponse($clean['response']);
    ...
    public static function err_from_map($m): ?KekkaiCurrencyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KekkaiCurrencyError(is_string($code) ? $code : '', $msg);
    }
