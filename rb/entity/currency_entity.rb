...
class CurrencyEntity
  ...
  def data_set(args)
    if args
      @_data = KekkaiCurrencyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = KekkaiCurrencyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [CurrencyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Currency.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Currency, Hash] the loaded Currency; raises KekkaiCurrencyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = KekkaiCurrencyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
