...
class MetadataEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KekkaiCurrencySDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KEKKAI_CURRENCY_TEST_METADATA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def metadata_basic_setup(extra)
  ...
  entid_env_raw = ENV["KEKKAI_CURRENCY_TEST_METADATA_ENTID"]
  ...
  if env["KEKKAI_CURRENCY_TEST_LIVE"] == "TRUE"
    ...
    client = KekkaiCurrencySDK.new(Helpers.to_map(merged_opts))
  end
