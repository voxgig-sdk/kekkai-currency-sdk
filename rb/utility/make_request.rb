...
module KekkaiCurrencyUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KekkaiCurrencyResponse.new({})
    ...
    if fetch_err
      ...
      response = KekkaiCurrencyResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KekkaiCurrencyResponse.new(fetched)
    else
