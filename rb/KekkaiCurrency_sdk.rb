...
require_relative 'KekkaiCurrency_types'


class KekkaiCurrencySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KekkaiCurrencyHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KekkaiCurrencyHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KekkaiCurrencyFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KekkaiCurrencyUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KekkaiCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KekkaiCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KekkaiCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => KekkaiCurrencyError.new(
        "#{op}_allow",
        "KekkaiCurrencySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue KekkaiCurrencyError => err
    ...
    if fetched.is_a?(Hash)
      status = KekkaiCurrencyHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KekkaiCurrencyError.new(
        "graphql_error", "KekkaiCurrencySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = KekkaiCurrencySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
