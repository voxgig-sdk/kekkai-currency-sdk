# KekkaiCurrency SDK test runner
...
class KekkaiCurrencyTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KekkaiCurrencyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KekkaiCurrencyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KekkaiCurrencyTestRunner.getenv("KEKKAI_CURRENCY_TEST_LIVE")
        override = KekkaiCurrencyTestRunner.getenv("KEKKAI_CURRENCY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KekkaiCurrencyTestRunner.getenv(key)
        ...
        explain = KekkaiCurrencyTestRunner.getenv("KEKKAI_CURRENCY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KekkaiCurrencyTestRunner._test_control is not None:
            return KekkaiCurrencyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KekkaiCurrencyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KekkaiCurrencyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KekkaiCurrencyTestRunner.load_test_control()
...
def load_env_local():
    KekkaiCurrencyTestRunner.load_env_local()


def env_override(m):
    return KekkaiCurrencyTestRunner.env_override(m)


def entity_data(v):
    return KekkaiCurrencyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KekkaiCurrencyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KekkaiCurrencyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KekkaiCurrencyTestRunner.load_test_control()


def live_client_options():
    return KekkaiCurrencyTestRunner.live_client_options()


def live_delay_ms():
    return KekkaiCurrencyTestRunner.live_delay_ms()
