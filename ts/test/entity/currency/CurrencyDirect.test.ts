...
describe('CurrencyDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new KekkaiCurrencySDK({
  ...
  test('direct-load-currency', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-currency', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KEKKAI_CURRENCY_TEST_CURRENCY_ENTID']
