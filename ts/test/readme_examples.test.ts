...
//     (\`new KekkaiCurrencySDK(...)\` / \`KekkaiCurrencySDK.test(...)\` become
//     \`KekkaiCurrencySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'KekkaiCurrencySDK'
...
const TEST_SEED = {"entity":{"chart":{"test01":{"id":"test01"}},"currency":{"test01":{"id":"test01"}},"metadata":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KekkaiCurrencySDK(...)\` and any
// \`KekkaiCurrencySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KekkaiCurrencySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KekkaiCurrencySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
