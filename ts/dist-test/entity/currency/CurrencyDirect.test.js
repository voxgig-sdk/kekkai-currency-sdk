...
(0, node_test_1.describe)('CurrencyDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.KekkaiCurrencySDK({
    ...
    (0, node_test_1.test)('direct-load-currency', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-currency', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.KEKKAI_CURRENCY_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.KekkaiCurrencySDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['KEKKAI_CURRENCY_TEST_CURRENCY_ENTID'];
    ...
    const client = new __1.KekkaiCurrencySDK({
        base: 'http://localhost:8080',
