// Command KekkaiCurrency-cli is a CLI + REPL for the KekkaiCurrency SDK,
...
const prompt = "kekkai-currency"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "chart currency metadata"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KEKKAI_CURRENCY_APIKEY (api key), KEKKAI_CURRENCY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KEKKAI_CURRENCY_APIKEY carries the API key and
	// KEKKAI_CURRENCY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KEKKAI_CURRENCY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KEKKAI_CURRENCY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKekkaiCurrencySDK(opts)
