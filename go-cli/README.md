...
boru-driven command-line client **and** interactive REPL for the KekkaiCurrency
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kekkai-currency-cli)
make build

# 2. See usage (words, entities, env vars)
./kekkai-currency-cli --help

# 3. Provide credentials once, via the environment
export KEKKAI_CURRENCY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kekkai-currency-cli list chart
./kekkai-currency-cli list currency

# 5. Override the API base URL for a single call
KEKKAI_CURRENCY_BASE=https://api.example.com ./kekkai-currency-cli list chart

# 6. No arguments -> interactive REPL
./kekkai-currency-cli
kekkai-currency> list chart
kekkai-currency> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kekkai-currency-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kekkai-currency-cli list chart
   ```
...
Run with no arguments to open a REPL (prompt `kekkai-currency>`). Each line is
evaluated as its own boru expression:

```text
$ ./kekkai-currency-cli
kekkai-currency> list chart
kekkai-currency> /help
kekkai-currency> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kekkai-currency-cli`. |
...
(`./kekkai-currency-cli <expr>`) and interactively (the REPL), and expressions compose the
